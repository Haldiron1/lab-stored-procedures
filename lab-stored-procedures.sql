...
CREATE PROCEDURE GetCustomersRentingCategoryMovies(IN category_name VARCHAR(255))
BEGIN
    SELECT 
        first_name, 
        last_name, 
...
CALL GetCustomersRentingCategoryMovies('action');
CALL GetCustomersRentingCategoryMovies('animation');
CALL GetCustomersRentingCategoryMovies('classics');
...
CREATE PROCEDURE FilterCategoriesByMovieCount(IN min_movie_count INT)
BEGIN
    SELECT 
        c.name AS category_name,
        COUNT(f.film_id) AS number_of_movies
    ...
    GROUP BY 
        c.name
    HAVING 
        COUNT(f.film_id) > min_movie_count;
END //
